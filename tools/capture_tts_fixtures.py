...
pick the two biggest major characters, calls ``cast-canonicalize`` for those two, and writes
the full response envelopes as fixtures. Tests assert schema/shape only, so a re-capture that
changes wording stays green (CLAUDE.md: never assert exact LLM content).
...
async def main() -> int:
    ...
    print(f"captured {len(pages)} mention pages + {len(majors)} canonicalizations")
    return 0
