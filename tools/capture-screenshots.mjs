...
/** Clip from the top through the bottom of the first element whose text starts with `endsAt`. */
async function clipThrough(page, endsAt) {
  return page.evaluate((needle) => {
    const el = [...document.querySelectorAll("section,div,form,table")]
      .find((n) => n.textContent.trim().startsWith(needle));
    if (!el) return null;
    return { x: 0, y: 0, width: window.innerWidth, height: Math.ceil(el.getBoundingClientRect().bottom + 24) };
  }, endsAt);
}
...
async function captureAdmin(ctx) {
  ...
  const routes = [
    ["01-books-list", "#/", null],
    ["02-new-book-wizard", "#/new", null],
    // Stop before the "delete this book permanently" panel — the milestones are the subject.
    ["03-book-detail", `#/book/${BOOK}`, "CONTROLS"],
    ["04-review-gate", `#/book/${BOOK}/review`, null],
    ["05-portrait-review", `#/book/${BOOK}/portraits`, null],
    ["06-post-render", `#/book/${BOOK}/postrender`, null],
  ];
  for (const [name, hash, endsAt] of routes) {
    if (!wants(name)) continue;
    await page.setViewportSize({ width: W, height: H });
    await page.goto(`${BASE}/admin/${hash}`, { waitUntil: "networkidle" });
    await page.waitForTimeout(1400);
    await fit(page);
    const clip = endsAt ? await clipThrough(page, endsAt) : null;
...
async function captureReader(ctx) {
  ...
  if (await picker.count()) {
    if (wants("07-profile-picker")) { await fit(page); await shot(page, "07-profile-picker"); }
    await page.setViewportSize({ width: W, height: H });
  ...
  if (wants("08-shelf")) { await fit(page); await shot(page, "08-shelf"); }
  await page.setViewportSize({ width: W, height: H });
...
try {
  await captureAdmin(ctx);
  await captureReader(ctx);
} finally {
  await browser.close();
}
