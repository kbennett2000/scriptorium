...
class SetRender:
    ...
    def units(self, job: Job, cfg: Any) -> list[Unit]:
        ...
        picture_ids = [*_page_plate_ids(cfg, job), COVER_ID, *portraits]
        return [
    ...
    async def _render(self, cfg: Any, job: Job, plate_id: str) -> None:
        ...
        spec = _asset_spec(_set_dir(cfg, job), plate_id)
        seed = _set_seed(job.book_id, job.source["set_id"], plate_id)
        await render_to_spec(
            self._client(cfg), wrapped, negative, spec, seed, style.get("imagegen_style")
        )
        _write_json(
            _set_dir(cfg, job) / "prompts" / f"{plate_id}.json",
            {
                "plate_id": plate_id,
