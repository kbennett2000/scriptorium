...
def fetch_text(gutenberg_id: int, *, client: httpx.Client) -> tuple[str, dict]:
    """Fetch a book's plain text + metadata from Gutendex (§5.1)."""
    meta_resp = client.get(f"{GUTENDEX_BASE}/books/{gutenberg_id}")
...
def load(spec: SourceSpec) -> RawBook:
    ...
    if spec.text is not None:
        ...
        with httpx.Client(timeout=30.0, follow_redirects=True) as client:
            raw, meta = fetch_text(spec.gutenberg_id, client=client)
